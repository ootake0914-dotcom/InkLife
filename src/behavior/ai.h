...
typedef uint8_t (*Scorer)(const Creature&);

inline uint8_t sSleep(const Creature& c) {
  uint8_t s = c.energy < 25 ? 90 : c.energy < 50 ? 55 : (c.energy > 80 ? 5 : 10);
  if (c.action == Action::SLEEP) s = min(100, (int)s + 20);  // 睡眠継続バイアス
  if (creatureNight(c.age_sec)) s = min(100, (int)s + 20);  // 夜は眠い (M10概日)
  return s;
}
inline uint8_t sEat(const Creature& c) {
  // M2は餌無限のデモ。M7でFOOD在庫と連動させる。
  return c.hunger > 75 ? 85 : c.hunger > 50 ? 55 : 10;
}
inline uint8_t sPlay(const Creature& c) {
  if (c.energy < 30) return 5;
  int s = c.happiness < 40 ? 60 : 20;
  if (creatureNight(c.age_sec)) s -= 10;  // 夜更かし抑制 (M10概日)
  if (s < 0) s = 0;
  return (uint8_t)s;
}
inline uint8_t sExplore(const Creature& c) {
  if (c.energy < 50) return 5;
  int s = 20 + c.curiosity / 3;  // 好奇心が探検率になる
  if (creatureNight(c.age_sec)) s -= 10;  // 夜の探検は控えめ
  else s += 10;  // 昼は活発
  if (s < 0) s = 0;
  if (s > 100) s = 100;
  return (uint8_t)s;
}
inline uint8_t sComm(const Creature& c) {
  // M6で他個体が居る時だけ高くする。今は顔見知りゼロなので低空飛行。
  return c.sociability / 4;
}
inline uint8_t sSeek(const Creature&) { return 0; }   // M7予約
inline uint8_t sFlee(const Creature&) { return 0; }   // M7予約
inline uint8_t sApproach(const Creature&) { return 0; }  // M7予約
inline uint8_t sFight(const Creature& c) {
  // 知人の有無はdecide()側で見る (peers==0なら0に潰す)。
  if (c.aggression > 60 && c.health > 70) return 45;
  return 0;
}
inline uint8_t sBreed(const Creature&) { return 0; }  // M8予約
...
inline Action decide(const Creature& c, uint8_t peers = 0) {
  Action best = Action::IDLE;
  uint8_t bestScore = 30;  // IDLEの基礎点。何もなければのんびり
  uint8_t curScore = 0;
  for (unsigned i = 0; i < sizeof(TABLE) / sizeof(TABLE[0]); i++) {
    uint8_t s = TABLE[i].fn(c);
