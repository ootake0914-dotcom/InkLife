#!/usr/bin/env python3
"""jpg2ink.py — JPG/PNG → E-Ink用1bit Cヘッダ変換 (GxEPD2 drawXBitmap互換XBM)。

使い方:
  python tools/jpg2ink.py input.jpg --name hero --size 96 -o src/display/art/art_hero.h
  python tools/jpg2ink.py input.jpg --preview-only   # 変換プレビューのみ
  python tools/jpg2ink.py --morphs                   # assets形態マスタ5感情分を一括生成

最適化: autocontrast → dither(既定atkinson) → XBM配列のみ出力。
RLEはFW・PCとも未参照のため既定で出さない (--with-rleで復活可)。

マスタ約束:
- 汎用5枚は形態絵の別名マスタ (ink_eat/sad/sleep/happy=m02_*, ink_idle=m01_*)。
  素体ヘッダ (art_ink_{idle,happy,eat,sleep,sad}.h) はここから個別生成する。
  ink_play.jpgは対応ヘッダなしの孤児 (PLAYはhappy絵で代用)。ink_greet.jpgはPC用に保持。
- 手続き生成 (gen_morphs.py) は凍結中。現行マスタを上書きしないこと。
"""
import argparse
import os
import sys
...
def convert_image(input_path, name=None, output_path=None, size=96,
    ...
    if output_path is None and not preview_only:
        output_path = os.path.join("src", "display", "art", f"art_{name}.h")

    img = Image.open(input_path).convert("RGB")
    # マスタ検証 (非正方・小解像度はXBM劣化の元。警告のみで続行)
    mw, mh = img.size
    if max(mw, mh) > min(mw, mh) * 1.5:
        print(f"警告: 非正方マスタ {mw}x{mh} (中央切抜で両端が落ちる)")
    if min(mw, mh) < 96:
        print(f"警告: 小解像度マスタ {mw}x{mh} (96pxへの拡大で線が太る)")
    ...
    raw = to_bitmap(bw)
    print(f"[{name}] master={mw}x{mh} size={size}x{size} raw={len(raw)}B "
          f"dither={dither}{' auto-bg-inv' if auto_inv else ''}")
    ...
    with open(output_path, "w") as f:
        ...
        f.write(f"// {size}x{size} XBM. 黒=描画。(RLEは未使用のため出力しない)\n\n")
...
def main():
    ...
    ap.add_argument("--morphs", action="store_true",
                    help="assets/ink_m*.jpg (5感情分) を自動走査して形態ヘッダーを一括生成。"
                         " 既存ヘッダーを上書きするため設定を合わせて使うこと")
    ap.add_argument("--with-rle", action="store_true",
                    help="RLE配列も併記する (既定OFF。FW・PCとも未参照)")
    ...
    if a.morphs:
        files = sorted(glob.glob(os.path.join("assets", "ink_m*.jpg")))
        if not files:
            print("形態画像が見つかりません: assets/ink_m*.jpg")
            return
        print(f"形態マスタ {len(files)}件を処理します")
        for fpath in files:
            base = os.path.splitext(os.path.basename(fpath))[0]
            out = os.path.join("src", "display", "art", f"art_{base}.h")
